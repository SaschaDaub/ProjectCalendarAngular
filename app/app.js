...
app.post("/get_polls", (request, response) => {
    accessDatabase("SELECT * FROM dbo.kp_polls")
        .then(function (res) {
            if (res.recordset.length > 0) {
                response.send(res.recordset);
            } else {
...
app.post("/get_polls_options", (request, response) => {
    accessDatabase("SELECT * FROM dbo.kp_polloptions")
        .then(function (res) {
            if (res.recordset.length > 0) {
                response.send(res.recordset);
            } else {
...
app.post("/save_polls", (request, response) => {
    newPoll = request.body.newPolls[0];
    editedPolls = request.body.editedPolls;
    newPollsOptions = request.body.newPollsOptions;

    if (newPoll) {
        accessDatabase("INSERT INTO dbo.kp_polls (name) VALUES ('" + newPoll.name + "')")
            .then(function () {
                response.send(true);
            }).catch(function (err) {
            throw err;
    ...
    if (newPollsOptions) {
        newPollsOptions.forEach(function (option, index) {
            accessDatabase("INSERT INTO dbo.kp_polloptions (startdate, enddate, pollfk, count, name) VALUES ('" + option.start + "', '" + option.end + "', " + newPoll.id + ", " + option.count + ", '" + option.name + "')")
                .then(function () {
...
app.post("/save_vote", (request, response) => {
	var option = request.body.option;
    accessDatabase("UPDATE dbo.kp_polloptions SET count=" + option.count + " WHERE id=" + option.id)
        .then(function () {
